...
func _get_actions() -> Array[GameController.GameAction]:
	...
	if _can_move(target_position):
		actions.append(GameController.GameAction.new(owned_by.id, GameController.eActionType.Move, pos, target_position))
	...
	if _can_attack(target_position):
		actions.append(GameController.GameAction.new(owned_by.id, GameController.eActionType.AttackMove, pos, target_position))
